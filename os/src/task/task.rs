...
impl TaskControlBlock {
    ...
    pub fn mmap(&self,start: usize, len: usize, port: usize) -> isize {
        ...
        for virt_page in VPNRange::new(start_va.floor(),end_va.ceil()){
            if let Some(pte)=inner.memory_set.translate(virt_page){
                if pte.is_valid(){
                    return -1;
                }
            }
    ...
    pub fn ummap(&self,start: usize, len: usize)->isize{
        ...
        for virt_page in VPNRange::new(start_va.floor(),end_va.ceil()){
            inner.memory_set.remove_area_with_start_vpn(virt_page);
        }

        0
    }
    ///spawn=fork+exec  把fork的代码copy过来，对着exec乱缝- -
    pub fn my_spawn(self:&Arc<Self>,elf_data: &[u8])->Arc<Self>{
