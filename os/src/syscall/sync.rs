...
pub fn sys_mutex_create(blocking: bool) -> isize {
        ...
        .map(|(id, _)| id)
    {
        ...
        process_inner.mutex_remain[id]=1;
        ...
        process_inner.mutex_remain.push(1);
        for tid in 0..process_inner.tasks.len() {
            let task = process_inner.get_task(tid);
            let mut task_inner = task.inner_exclusive_access();
...
pub fn sys_mutex_lock(mutex_id: usize) -> isize {
    ...
    if process_inner.deadlock_detect{
        let mut work=process_inner.mutex_remain.clone();
        let size=process_inner.tasks.len();
        let mut finish=vec![false;size];
        loop{
            let mut flag=false;
            for tid in 0..size{
                let task=process_inner.get_task(tid);
    ...
    let mut task_inner = task.inner_exclusive_access();
    process_inner.mutex_remain[mutex_id] -= 1;
...
pub fn sys_semaphore_create(res_count: usize) -> isize {
        ...
        .map(|(id, _)| id)
    {
        ...
        process_inner.sem_remain[id] = res_count;
        for tid in 0..process_inner.tasks.len() {
            let task = process_inner.get_task(tid);
            let mut task_inner = task.inner_exclusive_access();
        ...
        process_inner.sem_remain.push(res_count);
        for tid in 0..process_inner.tasks.len() {
            let task = process_inner.get_task(tid);
            let mut task_inner = task.inner_exclusive_access();
...
pub fn sys_semaphore_up(sem_id: usize) -> isize {
    ...
    process_inner.sem_remain[sem_id] += 1;
...
pub fn sys_semaphore_down(sem_id: usize) -> isize {
    ...
    if process_inner.deadlock_detect {
        //
        let mut work = process_inner.sem_remain.clone();
    ...
    if process_inner.sem_remain[sem_id] > 0 {
        process_inner.sem_remain[sem_id] -= 1;
        let mut task_inner = task.inner_exclusive_access();
        task_inner.sem_alloc[sem_id] += 1;
        task_inner.sem_need[sem_id] -= 1;
